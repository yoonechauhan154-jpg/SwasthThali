...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem('swasth_meal_logs', JSON.stringify(mealLogs));
  }, [mealLogs]);
  ...
  const handleSelectFooterDish = (dishName: string) => {
    setSearchQuery(dishName);
    setActiveTab('database');
  };

  const totalTodayCalories = (mealLogs || []).reduce((acc, curr) => acc + (curr?.totalCalories || 0), 0);
